...
function CustomError(message, name, extra)
	{
		if (extra == 'tinyMCE')
		{
			tinyMCE.getInstanceById(name).getWin().document.body.style.backgroundColor='white';
			if (tinyMCE.getContent() == '')
			{
			tinyMCE.getInstanceById(name).getWin().document.body.style.backgroundColor='#FFDFE6';
			document.getElementById('errors').innerHTML += "<div class='ErrorMessage'>" + message + "</div>"; return 1;
			}
			return 0;
		}
		else
...
function extra_validate(name,check_this) { 
	...
	if(check_this == 'apple_email') {
		...
		if (inv_email == true) { return "You can only enter vaild Apple Email address(s) into the CC Field"; }
	}
