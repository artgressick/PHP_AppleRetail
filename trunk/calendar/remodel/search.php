...
<?
			...
			$q = "SELECT DISTINCT ID, chrKEY, chrTitle FROM NSOLearn WHERE !bDeleted AND idType=2 AND bShow AND bPShow AND (LCASE(txtContent)LIKE LCASE('%".encode($_POST['chrSearch'])."%') OR LCASE(chrTitle) LIKE LCASE('%".encode($_POST['chrSearch'])."%')) ORDER BY chrTitle";
