				...
				<tr>
					...
					<td style='text-align: right;'>
						<?=form_select($filter_results,array('nocaption'=>'true',
									'name'=>'idNSOType',
									'caption'=>'- NSO Types -',
									'value'=>(isset($_REQUEST['idNSOType']) && is_numeric($_REQUEST['idNSOType']) ? $_REQUEST['idNSOType'] : ''),
...
<?
	...
	if(isset($_REQUEST['idNSOType']) && is_numeric($_REQUEST['idNSOType'])) {	
		$tableHeaders = array(
			'chrNSOCorpTask'			=> array('displayName' => 'Task'),
			'chrNSOType'			=> array('displayName' => 'Event Type'),
			'intDateOffset'			=> array('displayName' => 'Day Offset')
		);
		if(access_check(41,3)) {
			$tableHeaders['opt_other'] = 'order';
		}
		if(access_check(41,4)) {
			$tableHeaders['opt_del'] = 'chrNSOCorpTask';
		}
		...
		sortList('NSOCorpTasks',		# Table Name
			$tableHeaders,			# Table Name
			$results,				# Query results
			(access_check(41,3) ? 'edit.php?key=' : ''),		# The linkto page when you click on the row
