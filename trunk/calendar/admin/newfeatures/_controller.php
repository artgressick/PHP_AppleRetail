<?
	...
	switch($file_name[0]) {
		...
		case 'index.php':
			...
			auth_check('litm',42,3);
			include($BF.'calendar/components/formfields.php');
			
			
			# Check for KEY, if not Error, Get $info, Error if no results

			$info = db_query("SELECT txtAnnoucement, bShow
								FROM NSONewFeatures
								WHERE ID=1","getting info",1); // Get Info
				
			
			if(isset($_POST['txtAnnoucement'])) { include($post_file); }
...
//		total += CustomError('You must fill in the Landing Page', 'txtContent', 'tinyMCE');
		...
		if(total == 0) { notice('New Features Popup', 'index.php'); }
	}
</script>
