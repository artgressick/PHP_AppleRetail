<?
	...
	switch($file_name[0]) {
		...
		case 'index.php':
			...
			$q = "SELECT ID,chrKEY,chrCat,intOrder FROM EvalCats WHERE !bDeleted ORDER BY intOrder,chrCat";
			$results = db_query($q,"getting Categories");
			
			if(count($_POST)) { include($post_file); }
			
			# The template to use (should be the last thing before the break)
			$title = "Evaluation Categories";						# Page Title
			$directions = 'Choose a Category from the list below. Click on the column header to sort the list by that column.';
			$header_title = (access_check(29,2) ? linkto(array('address'=>'add.php','img'=>'/calendar/images/plus_add.png')):'').' Evaluation Categories <span class="resultsShown">(<span id="resultCount">'.mysqli_num_rows($results).'</span> results)</span>';
