<?
	...
	switch($file_name) {
		...
		case 'index.php':
			...
			$q = "SELECT Evals.ID,Evals.chrKEY,Evals.dtCreated,CONCAT(chrLast,', ',chrFirst) as chrName
				FROM Evals
				JOIN Users ON Users.ID=Evals.idUser
				WHERE !Evals.bDeleted
				ORDER BY ID DESC";
			$results = db_query($q,"getting NSO File Groups");
	
			# The template to use (should be the last thing before the break)
			$title = "Evaluations";						# Page Title
			$directions = 'Choose an Evaluation from the list below. Click on the column header to sort the list by that column.';
			$header_title = 'Evaluations <span class="resultsShown">(<span id="resultCount">'.mysqli_num_rows($results).'</span> results)</span>';
