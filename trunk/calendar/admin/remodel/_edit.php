<?
	...
	if($info['bParent']) {
		...
		if($_POST['idParent'] != $info['idParent']) {
			...
			$newparent = db_query("SELECT dOrder, bPShow FROM NSOLearn WHERE idType=2 AND bParent AND ID=".$_POST['idParent'],"Getting New Parent Information",1);
			$_POST['bPShow'] = $newparent['bPShow'];
			$_POST['dOrder'] = $newparent['dOrder']; 
			$highest = db_query("SELECT dOrderChild FROM ".$table." WHERE !bParent AND idType=2 AND idParent='".$_POST['idParent']."' AND ID != '".$info['ID']."' AND !bDeleted ORDER BY dOrderChild DESC","Getting Highest Number",1);
