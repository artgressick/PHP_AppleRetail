<?
	...
	if(db_query($q,"Insert into ". $table)) {
		...
		if($_POST['bParent']) { // Parent
			...
			$highest = db_query("SELECT dOrder FROM ".$table." WHERE bParent AND idType=2 AND ID != '".$newID."' AND !bDeleted ORDER BY dOrder DESC","Getting Highest Number",1);
			...
			$parentinfo = db_query("SELECT bPShow, dOrder FROM NSOLearn WHERE bParent AND idType=2 AND ID=".$_POST['idParent'],"Getting Parent Information",1);
			$highest = db_query("SELECT dOrderChild FROM ".$table." WHERE !bParent AND idType=2 AND idParent='".$_POST['idParent']."' AND ID != '".$newID."' AND !bDeleted ORDER BY dOrderChild DESC","Getting Highest Number",1);
