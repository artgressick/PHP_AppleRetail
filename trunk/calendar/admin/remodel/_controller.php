<?
	...
	switch($file_name[0]) {
		...
		case 'index.php':
			...
			$title = "About Remodels Articles";						# Page Title
			$directions = 'Choose a article from the list below. Click on the column header to sort the list by that column.';
			$header_title = (access_check(18,2) ? linkto(array('address'=>'add.php','img'=>'/calendar/images/plus_add.png')) : '').' About Remodels Articles <span class="resultsShown">(<span id="resultCount">'.mysqli_num_rows($results).'</span> results)</span>';
...
<?
			...
			# The template to use (should be the last thing before the break)
			$title = "Add About Remodels Article";	# Page Title
			$directions = 'You are adding a About Remodels Article to the database. It is <strong>not</strong> suggested to use hard widths, but instead use percentages unless unavoidable.<br /><strong>NOTE: Please Use FireFox to add and edit Articles as some features may not be available or function properly.</strong>';
			$header_title = 'Add About Remodels Article';
			include($BF ."calendar/models/admin.php");	
...
<?
			...
			# The template to use (should be the last thing before the break)
			$title = "Edit About Remodels Article";	# Page Title
			...
			$header_title = 'Edit About Remodels Article: '. $info['chrTitle'];
			include($BF ."calendar/models/admin.php");		
