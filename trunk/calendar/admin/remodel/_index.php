		...
		if($row['bParent']) {
			if($row['dOrder'] != $_POST['dOrder'.$row['ID']]) {
				if(is_numeric($_POST['dOrder'.$row['ID']])) { 
					if(db_query("UPDATE ".$table." SET dOrder='".$_POST['dOrder'.$row['ID']]."' WHERE idType=2 AND idParent=".$row['ID'],"Updating Parent Order")) { $count++; }
				} else {
			...
			if($row['dOrderChild'] != $_POST['dOrderChild'.$row['ID']]) {
				if(is_numeric($_POST['dOrderChild'.$row['ID']])) { 
					if(db_query("UPDATE ".$table." SET dOrderChild='".$_POST['dOrderChild'.$row['ID']]."' WHERE idType=2 AND ID=".$row['ID'],"Updating Child Order")) { $count++; }
				} else {
					$_SESSION['errorMessages'][] = "You must enter a number for the order of ".$row['chrTitle'].". Order value reset.";
				}
			}
