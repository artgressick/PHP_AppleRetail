	...
	switch($file_name[0]) {
		...
		case 'index.php':
			...
			$info = db_query("SELECT txtHTML
								FROM LandingPage
								WHERE ID=11","getting info",1); // Get Info
...
//		total += ErrorCheck('chrEmailCategory', "You must enter an Email Category Name.");
		total += CustomError('You must fill in the Content for the First Time Login Page', 'txtContent', 'tinyMCE');

		if(total == 0) { notice('First Time Login Page', 'index.php'); }
	}
</script>
