...
function form_text($args) {
	...
	if(is_array($args)) { 
		...
		if(isset($args['display'])) { 
			return $tmp."<div class='FormDisplay' style='".$style."'>".$value."</div>";
		} else {
			return $tmp."<".(isset($args['nocaption'])?"span":"div")." class='FormField'><input type='".(isset($args['type'])?$args['type']:'text')."' name='".$name."' id='".$id."'".($size!=''? " size='".$size."'" :'').($maxlength!=''? " maxlength='".$maxlength."'" :'').($title!=''? " title='".$title."'" :'').($value!=''? " value='".$value."'" :'').($class!=''? " class='".$class."'" :'').($style!=''? " style='".$style."'" :'').($extra!=''? " ".$extra :'')." /></".(isset($args['nocaption'])?"span":"div").">";
		}
...
function form_select($records,$args) {
	...
	##			 'style' => 'CSS Style',						// Extra Styles for Text Box
	##			 'extra' => 'Extra Code, Javascript, etc',		// For additional JS, to Disable Field, or any additional options can be entered here.
	##			 'forcetitle' => 'Title',						// This is used for the first select option.
	##			));
	####################################################################
	...
	if(is_array($args)) { 
		...
		if(isset($args['display'])) { 
			...
			if(isset($args['forcetitle']) && $args['forcetitle'] != '') {
				$tmp .= "<option value=''>".$args['forcetitle']."</option>";
			} else if(($value == '' && isset($args['required'])) || !isset($args['required'])) {
			...
			if(!is_array($records)) {
				while($row = mysqli_fetch_assoc($records)) {
					$row['chrRecord'] =	preg_replace('/<hidden>([^>]*)<\/hidden>/si', '', $row['chrRecord']);
					...
					$tmp .= "<option".($value != '' && $row['ID'] == $value ? ' selected="selected"' : '')." value='".$row['ID']."'>".$row['chrRecord']."</option>";
				}

			} else { 
				if(isset($args['type']) && $args['type'] == 'simplearray') {
					while ($array_value = current($records)) {
						$tmp .= "<option".($value != '' && $array_value == $value ? ' selected="selected"' : '')." value='".$array_value."'>".$array_value."</option>";		
						next($records);
					}
					...
					while ($array_value = current($records)) {
						...
						$tmp .= "<option".($value != '' && $array_value['value'] == $value ? ' selected="selected"' : '')." value='".$array_value['value']."'>".$array_value['display']."</option>";		
						next($records);
					}
				} else {
					while ($array_value = current($records)) {
						$tmp .= "<option".($value != '' && key($records) == $value ? ' selected="selected"' : '')." value='".key($records)."'>".$array_value."</option>";		
						next($records);
					}
				}
			}
...
function form_checkbox($args) {
	...
	##			 'value' => encode(Field Value),				// Value of the field, be sure to encode this
	##			 'name' => 'Field Name',						// Name of field for Post, (ie. chrFirst)
	##			 'nodisplay' =>	'No Display',					// Display nothing BUT the checkbox itself.
	##			 'id' => 'Field ID',							// ID of field for JS, (ie. chrFirst)
	...
	if(is_array($args)) { 
		...
		$tmp .= "<span><input type='".$type."' name='".$name.(isset($args['array']) && $type != 'radio' ? '[]' : '')."' id='".$id."'".(isset($args['checked']) && $args['checked'] == 'true' ? ' checked="checked"':'').($title!=''? " title='".$title."'" :'').($value!=''? " value='".$value."'" :'').($class!=''? " class='".$class."'" :'').($style!=''? " style='".$style."'" :'').($extra!=''? " ".$extra :'')." />";
		if(!isset($args['nodisplay'])) {
			$tmp .= "<label for='".$id."'>".($caption != "" ? addslashes($title) : '')."</label></span>";
		}
		return $tmp;
		if($name == "") { 
...
function form_textarea($args) {
	...
	if(is_array($args)) { 
		...
		return "<div class='FormName'>".$caption . $required."</div><textarea name='".$name."' id='".$id."'".($rows!=''? " rows='".$rows."'" :'').($cols!=''? " cols='".$cols."'" :'').($title!=''? " title='".$title."'" :'').($class!=''? " class='".$class."'" :'').($style!=''? " style='".$style."'" :'').($extra!=''? " ".$extra :'').">".$value."</textarea>";
		if($name == "") { 
