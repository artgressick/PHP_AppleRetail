...
<?		$count = 0;
		if(mysqli_num_rows($query)) { 
			...
			if(preg_match('/(\?|\&)key\=/',$linkto)) {
				$linktype = 'chrKEY'; 
			} else if(preg_match('/(\?|\&)id\=/',$linkto)) { 
				$linktype = 'ID';
			} else { $linktype = ''; }
...
<?
			...
			?>			<td class='options'><? deleteButton($row['ID'],$displayVal,$row['chrKEY'],$table); ?></td> 		<?
				...
				if ($v == 'order') {
?>					<td><?=orderBoxes($row['ID'],(isset($row['dOrder']) ? $row['dOrder'] : $row['intOrder']))?></td>
<?
				} else if ($v == 'quantity') {
?>					<td><?=quantityBoxes($row['ID'],$row['intQuantity'],$db_table)?></td>
<?
				}
