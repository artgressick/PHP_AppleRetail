...
<?
			$temp = db_query("SELECT idSSQuestion, txtAnswer FROM SSAnswers WHERE idNSOSS=".$info['ID'],"Getting Answers");
			$answers = array();
			while($row = mysqli_fetch_assoc($temp)) {
				$answers[$row['idSSQuestion']] = $row['txtAnswer'];
			}

			$results = db_query("SELECT Q.ID,Q.chrQuestion,EC.chrCat,Q.idSSCat
				FROM SSQuestions AS Q
				JOIN SSCats AS EC ON Q.idSSCat=EC.ID
				WHERE Q.idSS = ".$info['idSS']."
				ORDER BY EC.intOrder, EC.chrCat,Q.dOrder,Q.idSSType,Q.chrQuestion
			","Getting Event info");
			
			$prevCat = '';
			
			while($row = mysqli_fetch_assoc($results)) {
						if($prevCat != $row['idSSCat']) {
				$prevCat = $row['idSSCat'];
?>
			<hr />		
