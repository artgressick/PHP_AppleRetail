<?php
	...
	if(count($_POST)) { // When doing isset, use a required field.  Faster than the php count funtion.
		...
		<br /><br />Region Name:||| ".encode($_POST['chrEmployeeName']).":::
		<br /><br />New Hire's Full Name:||| ".encode($_POST['chrEmployeeNumber']).":::
		<br /><br />Requisition Number:||| ".encode($_POST['chrEmployeeApple']).":::
		<br /><br />Effective Date on Merlin Offer:||| ".encode($_POST['chrEmployeeEmail']).":::
		<br /><br />Date Paperwork Sent In:||| ".encode($_POST['chrEmployeeTitle']).":::
		<br /><br />FEDEX Tracking Number, if available:||| ".encode($_POST['chrFEDEX']).":::
...
?>
	...
	<script language="javascript">
		function error_check() {
			...
			total += ErrorCheck('chrEmployeeNumber', "You must enter the New Hire's Full Name.");
			total += ErrorCheck('chrEmployeeEmail', "You must enter an effective date on Merlin offer.");
			total += ErrorCheck('chrEmployeeName', "You must enter the Region Name.");
			total += ErrorCheck('chrEmployeeApple', "You must enter a Requisition Number.");
			total += ErrorCheck('chrEmployeeTitle', "You must enter the date the paperwork was sent in.");
	...
	require($BF. "commtool/includes/top_email.php"); //Table started here, 3 columns wide.
	 ?>
					...
					<tr>
						...
						<td class="gutter"></td>
						<td class="right">
							<div class='FormName'>New Hire's Full Name: <span class='FormRequired'>(Required)</span></div>
					...
					<tr>
						...
						<td class="gutter"></td>
						<td class="right">
							<div class='FormName'>Effective Date on Merlin Offer: <span class='FormRequired'>(Required)</span></div>
							<div class='FormField'><input class="FormTextBox" type="text" name="chrEmployeeEmail" id="chrEmployeeEmail"  maxlength="50" /></div>
						</td>
					</tr>
					<tr>
						<td class="left">
							<div class='FormName'>Date Paperwork Sent In: <span class='FormRequired'>(Required)</span></div> 
							<div class='FormField'><input class="FormTextBox" type="text" name="chrEmployeeTitle" id="chrEmployeeTitle" maxlength="50" /></div>
						</td>
						<td class="gutter"></td>
						<td class="right">
						<div class='FormName'>FEDEX Tracking Number, if available: <span class='FormRequired'></span></div> 
