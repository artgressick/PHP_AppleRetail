...
?>
	...
	<script language="javascript">
		function error_check() {
			...
			total += ErrorCheck('chrStoreNumber', "記憶装置番号を入れなければならない");
			total += ErrorCheck('chrStore', "記憶装置の名前を入れなければならない.");
			if (document.getElementById('chrCC').value != '') { 
				total += ErrorCheck("chrCC","Apple の有効なEメールアドレスを入れなければならない。","apple_email");
			}
			total += ErrorCheck('chrEmployee', "従業員の名前を入れなければならない。");
			total += ErrorCheck('chrEmail', "従業員のEメールアドレスを入れなければならない。");
			total += ErrorCheck('chrAttachment', "接続機構を追加しなければならない。");
...
<?	include($BF. "includes/top.php");
	...
	include($BF. "commtool/includes/top_email_jp.php"); //Table started here, 3 columns wide. ?>
					<tr>
						<td class="left">
							<div class='FormName'>従業員の名前<span class='FormRequired'>(必須)</span></div> 
							<div class='FormField'><input class="FormTextBox" type="text" name="chrEmployee" id="chrEmployee" maxlength="50" /></div>
						</td>
						<td class="gutter"></td>
						<td class="right">
							<div class='FormName'>従業員のEメールアドレス<span class='FormRequired'>(必須)</span></div> 
					...
					<tr>
						<td colspan="3">
							<div class='FormName'>コメント<span class='FormRequired'>(このボックスにその他のメモを記入できます。)</span></div>
...
<?
	include($BF. "commtool/includes/bottom_email_jp.php");
