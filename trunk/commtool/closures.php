<?php
	...
	if(count($_POST)) { // When doing isset, use a required field.  Faster than the php count funtion.
		...
		<br /><br />Early Closure or Late Opening:||| ".$_POST['chrBelow'].":::
		<br /><br />Estimated Time for Re-Opening:||| ".encode($_POST['chrCheckNum']).":::
		<br /><br />Description of the Issue:||| ".nl2br(encode($_POST['Notes'])).":::";
		$msg = encode($msg);
...
?>
	...
	<script language="javascript">
		function error_check() {
			...
			total += ErrorCheck('chrCheckNum', "You must enter an estimated time for Re-Opening.");
			
			if (document.getElementById('bBelowYes').checked == false && document.getElementById('bBelowNo').checked == false) {
			
				total ++;
				document.getElementById('errors').innerHTML += "<div class='ErrorMessage'>You must indicate the reason for the closure.</div>";
...
<?	include($BF. "includes/top.php");
	...
	include($BF. "commtool/includes/top_email.php"); //Table started here, 3 columns wide. ?>
					...
					<tr>
						<td class="left">
							<div class='FormName'>Early Closure or Late Opening:<span class='FormRequired'> (Required)</span></div> 
							<div class='FormField' id="bBelow"><input type="radio" name="chrBelow" id="bBelowYes" value="Early Closure" /> Early Closure &nbsp; <input type="radio" name="chrBelow" id="bBelowNo" value="Late opening" /> Late Opening</div>
						</td>
						<td class="gutter"></td>
						<td class="right">
							<div class='FormName'>Estimated Time for Re-Opening:<span class='FormRequired'> (Required)</span></div>
							<div class='FormField'><input class="FormTextBox" type="text" name="chrCheckNum" id="chrCheckNum" maxlength="50" /></div>
						</td>
					</tr>
					<tr>
						<td colspan="3">
							<div class='FormName'>Description of the Issue:<span class='FormRequired'> (You can enter other notes in this box)</span></div>
