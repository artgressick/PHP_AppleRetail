<?php
	...
	(!isset($_REQUEST['d']) || $_REQUEST['d'] == "" ? ErrorPage("commtool/index.php","Invalid Page") : "");
	parse_str(base64_decode($_REQUEST['d']),$info);
	($info['id'] == "" || !is_numeric($info['id']) || $info['special'] != "011101" ? ErrorPage("commtool/index.php","Invalid Page") : "");
	$query ="SELECT EmailMessages.*, chrStatus, RFLEmails.chrRFLName as chrType, RFLEmails.chrPHPPage,chrStoreNum, RFLEmails.idGeo
	FROM EmailMessages
	...
	WHERE EmailMessages.ID = '".$info['id']."'";	
	$result = database_query($query, 'Get email', 1);
	
	(!is_numeric($result['ID']) ? ErrorPage("commtool/index.php","Invalid Page") : "");
	
	if (!isset($_SESSION['idGeo'])) { $_SESSION['idGeo'] = $result['idGeo']; } 
...
<a href='http://storeops.apple.com/commtool/status.php?d=".base64_encode("id=".$info['id']."&special=011101")."'>http://storeops.apple.com/commtool/status.php?d=".base64_encode("id=".$info['id']."&special=011101")."</a><br /><br />
...
//		$to = "jsummers@techitsolutions.com, ";
		...
		header('Location: status.php?d='.$_REQUEST['d']);
		die();
	}
