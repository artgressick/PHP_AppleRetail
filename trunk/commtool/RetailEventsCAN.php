...
/*  This is the query to get the distro list for the current e-mail.*/
	...
	if(count($_POST)) { // When doing isset, use a required field.  Faster than the php count funtion.
		...
		<br /><br />Article Name/Number:||| ".encode($_POST['chrArticle']).":::
...
?>
	...
	<script language="javascript">
		function error_check() {
			...
			total += ErrorCheck('chrEmployee', "You must enter a Employeee Name.");
			total += ErrorCheck('chrArticle', "You must enter an Article Number or Name.");
			total += ErrorCheck('Notes', "You must enter a Comment.");
...
<?	include($BF. "includes/top.php");
	...
	include($BF. "commtool/includes/top_email.php"); //Table started here, 3 columns wide. ?>
					<tr>
						...
						<td class="gutter"></td>
						<td class="right">
							<div class='FormName'>Article Name or Number <span class='FormRequired'>(Required)</span></div> 
							<div class='FormField'><input class="FormTextBox" type="text" name="chrArticle" id="chrArticle" maxlength="50" /></div>
						</td>
					</tr>
					<tr>
						<td colspan="3">
							<div class='FormName'>Comments <span class='FormRequired'>(Required) (You can enter other notes in this box)</span></div>
