<?php
	...
	if(count($_POST)) { // When doing isset, use a required field.  Faster than the php count funtion.
		...
		<br /><br />Part Number:||| ".encode($_POST['chrPart']).":::
		<br /><br />Ringing up as/at:||| ".encode($_POST['chrRinging']).":::
		<br /><br />Should Ring up as/at:||| ".encode($_POST['chrShould']).":::
		<br /><br />Other Notes:||| ".encode($_POST['Notes']).":::";
				$msg = encode($msg);
...
?>
	...
	<script language="javascript">
		function error_check() {
			...
			total += ErrorCheck('chrRinging', "You must enter a Ringing Up as Amount.");
			total += ErrorCheck('chrPart', "You must enter a Part Number.");
			total += ErrorCheck('chrShould', "You must enter a Should Ring up as Amount.");
...
<?	include($BF. "includes/top.php");
	...
	include($BF. "commtool/includes/top_email.php"); ?>
					...
					<tr>
						<td class="left">
							<div class='FormName'>Ringing up as/at <span class='FormRequired'>(Required)</span></div>
							<div class='FormField'><input class="FormTextBox" type="text" name="chrRinging" id="chrRinging" maxlength="50" /></div>
						</td>
						<td class="gutter"></td>
						<td class="right">
							<div class='FormName'>Part Number <span class='FormRequired'>(Required)</span></div>
							<div class='FormField'><input class="FormTextBox" type="text" name="chrPart" id="chrPart" maxlength="50" /></div>
						</td>
					</tr>
					<tr>
						<td>
							<div class='FormName'>Should Ring up as/at <span class='FormRequired'>(Required)</span></div> 
							<div class='FormField'><input class="FormTextBox" type="text" name="chrShould" id="chrShould" maxlength="50" /></div>
						</td>
						<td class="gutter"></td>
						<td class="right"></td>						
					</tr>
					<tr>
						<td colspan="3">
							<div class='FormName'>Extra Notes <span class='FormRequired'>(You can enter other notes in this box)</span></div>
