<?php
	...
	if(count($_POST)) { // When doing isset, use a required field.  Faster than the php count funtion.
		...
		<br /><br />Price Override Amount:||| ".encode($_POST['chrAmount']).":::
		<br /><br />Price Override Reason:||| ".encode($_POST['Notes']).":::";
				$msg = encode($msg);
...
?>
	...
	<script language="javascript">
		function error_check() {
			...
			total += ErrorCheck('chrAmount', "You must enter a Override Amount.");
			total += ErrorCheck('Notes', "You must enter a Override Reason.");
...
<?	include($BF. "includes/top.php");
	...
	include($BF. "commtool/includes/top_email.php"); ?>
					...
					<tr>
						...
						<td class="gutter"></td>
						<td class="right">
							<div class='FormName'>Price Override Amount <span class='FormRequired'>(Required)</span></div>
							<div class='FormField'><input class="FormTextBox" type="text" name="chrAmount" id="chrAmount" maxlength="50" /></div>
						</td>
					</tr>
					<tr>
						<td colspan="3">
							<div class='FormName'>Price Override Reason <span class='FormRequired'>(Required) (You can enter other notes in this box)</span></div>
