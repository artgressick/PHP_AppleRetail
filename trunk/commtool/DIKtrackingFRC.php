...
?>
	...
	<script language="javascript">
		function error_check() {
			...
			total += ErrorCheck('chrStoreNumber', "Vous devez écrire un nombre de mémoire.");
			total += ErrorCheck('chrStore', "Vous devez écrire un nom de mémoire.");
			if (document.getElementById('chrCC').value != '') { 
				total += ErrorCheck("chrCC","Vous devez écrire un email address de Apple de vaild.","apple_email");
			}
			total += ErrorCheck('chrEmployee', "Vous devez écrire un nom d'Employeee.");
			total += ErrorCheck('chrEmail', "Vous devez écrire un email d'Employeee.");
			total += ErrorCheck('chrAttachment', "Vous devez attacher un fichier.");
...
<?	include($BF. "includes/top.php");
	...
	include($BF. "commtool/includes/top_email_cafr.php"); //Table started here, 3 columns wide. ?>
					<tr>
						<td class="left">
							<div class='FormName'>Nom des Employés<span class='FormRequired'>(requis)</span></div> 
							<div class='FormField'><input class="FormTextBox" type="text" name="chrEmployee" id="chrEmployee" maxlength="50" /></div>
						</td>
						<td class="gutter"></td>
						<td class="right">
							<div class='FormName'>Email address des Employés <span class='FormRequired'>(requis)</span></div> 
					...
					<tr>
						<td colspan="3">
							<div class='FormName'>Commentaires <span class='FormRequired'>(vous pouvez saisir des notes additionnelles dans ce champ.)</span></div>
...
<?
	include($BF. "commtool/includes/bottom_email_cafr.php");
