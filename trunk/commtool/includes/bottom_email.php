				</table>
				<div class='FormName'><?=(isset($AttachText)?$AttachText : "Attach File")?></div>
				<div class='FormUploadFile'><input class="FormUploadFile" name='chrAttachment' id='chrAttachment' type="file" value="Choose a file" /></div> 
				<div class='FormButtons'><input style="margin-top:15px;" type="button" value="Send Email" onclick='error_check();' /></div>
				<input type="hidden" id="id" name="id" value="<?=$_REQUEST['id']?>" />
			</div>
		</td>
	</tr>
</table>
</form>