<?php
	...
	if(count($_POST)) { // When doing isset, use a required field.  Faster than the php count funtion.
		...
		<br /><br />Overage/Shortage Date:||| ".encode($_POST['chrDate']).":::
		<br /><br />POS Variance Amount:||| ".encode($_POST['chrAmount']).":::
		<br /><br />Variance Occured on Register Number:||| ".encode($_POST['chrShortageReg']).":::
		<br /><br />Explanation of Variance:||| ".encode($_POST['Notes']).":::";
				$msg = encode($msg);
...
?>
	...
	<script language="javascript">
		function error_check() {
			...
			total += ErrorCheck('chrDate', "You must enter a Overage/Shortage Date.");
			total += ErrorCheck('chrAmount', "You must enter a Variance Amount.");
			total += ErrorCheck('chrRegNumber', "You must enter a Register Number.");
			total += ErrorCheck('Notes', "You must enter a Explanation.");
...
<?	include($BF. "includes/top.php");
	...
	include($BF. "commtool/includes/top_email.php"); ?>
					...
					<tr>
						<td class="left">
							<div class='FormName'>Overage/Shortage Date <span class='FormRequired'>(Required)</span></div>
							<div class='FormField'><input class="FormTextBox" type="text" name="chrDate" id="chrDate" maxlength="50" /></div>
						</td>
						<td class="gutter"></td>
						<td class="right">
							<div class='FormName'>POS Variance Amount <span class='FormRequired'>(Required)</span></div>
							<div class='FormField'><input class="FormTextBox" type="text" name="chrAmount" id="chrAmount" maxlength="50" /></div>
						</td>
					</tr>
					<tr>
						<td>
							<div class='FormName'>Variance Occurred on Register Number <span class='FormRequired'>(Required)</span></div> 
							<div class='FormField'><input class="FormTextBox" type="text" name="chrRegNumber" id="chrRegNumber" maxlength="50" /></div>
						</td>
						<td class="gutter"></td>
						<td class="right"></td>						
					</tr>
					<tr>
						<td colspan="3">
							<div class='FormName'>Explanation of Variance <span class='FormRequired'>(Required) (You can enter other notes in this box)</span></div>
